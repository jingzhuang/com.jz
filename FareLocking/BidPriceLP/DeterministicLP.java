...
public class DeterministicLP
{
    ...
    public DeterministicLP( int solveTime_ )
    {
        try
        {
            ...
            bidPrices = new double[ DataKeeper.instance().getNoFares() ];
            construct();
        }
        catch ( Exception e )
        {
    ...
    public void computeBidPrices()
    {
        try
        {
            ...
            for ( int t = solveTime ; t < DataKeeper.instance().getNoTimePeriods() ; t ++ )
            {
                double dual = consts[ t ].get( GRB.DoubleAttr.Pi );
                for ( int f = 0 ; f < DataKeeper.instance().getNoFares() ; f ++ )
                {
                    bidPrices[ f ] += dual;
                    ...
                    double lp = DataKeeper.instance().getLockProb( f );
                    double pp = DataKeeper.instance().getPurchProb( f );
                    if ( t >= solveTime - 1 + hold )
                    {
                        bidPrices[ f ] -= ( lp * ( 1 - pp ) * dual );
                    }
    ...
    public double getBidPrice ( int fc )
    {
        return bidPrices[ fc ];
    }
